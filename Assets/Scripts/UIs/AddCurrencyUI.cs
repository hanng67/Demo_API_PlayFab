...
public class AddCurrencyUI : MonoBehaviour
{
    ...
    private void AddUserVirtualCurrency()
    {
        ...
        PlayFabClientAPI.AddUserVirtualCurrency(new AddUserVirtualCurrencyRequest
        {
            Amount = vcValue,
            VirtualCurrency = vcKey
        }, OnAddUserVirtualCurrencySuccess, OnRequestFailure);
    }

    private void OnAddUserVirtualCurrencySuccess(ModifyUserVirtualCurrencyResult result)
    {
        consoleUI.Write("Add User Currency Successfully:");
        consoleUI.WriteLine($"BalanceChange: {result.BalanceChange} \n" +
                            $"Balance: {result.Balance}");
        userBalanceUI.UpdateVirtualCurrency(vcKey, vcValue);
    }
